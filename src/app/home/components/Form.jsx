...
const Form = () => {
    ...
    const handleSubmit = async () =>  {
        ...
        if (description !== null && description.length > 0) {
            ...
            let response = axios.get(API_URL + '?input=' +  description).then(response => {offers = response})
    ...
    return (
        <div className="container mx-auto">
            ...
            <p className="text-xl font-bold my-3">Tell us about <span className="text-[#167DB7]">you</span></p>
            <div className="w-100 h-40 p-2 border-solid border-2 border-[#167DB7] rounded-xl">
                <textarea onChange={e => setDescription(e.target.value)} placeholder={placeholder} className="w-full h-full resize-none outline-0"></textarea>
            </div>
            {errorStatus && <div className="bg-red-100 border border-red-400 text-red-700 my-4 px-4 py-3 rounded" role="alert">
                    <strong class="font-bold">{errorMessage}</strong>
            </div>}
            <div className="flex flex-row items-center justify-center my-12">
                {!processing ? 
                    <button onClick={handleSubmit} className="bg-[#167DB7] py-2 px-8 rounded-xl">
                        <p className="font-bold text-white text-base">GO</p>
                    </button> : 
                    <button type="button" class="bg-[#167DB7] py-2 px-8 rounded-xl" disabled>
